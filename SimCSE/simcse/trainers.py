...
class CLTrainer(Trainer):
    ...
    def evaluate(
        ...
        # features = {k:self.train_dataset[k]  for k in self.train_dataset.column_names}
        ...
        #     print(len(i))
        batch = {k: batch[k].view(len(self.train_dataset), -1) for k in batch}
        
        train_labels = self.train_dataset["label"]
        self.model.eval()
        with torch.no_grad():
            base_embeddings = self.model(**batch, output_hidden_states=True,
                             return_dict=True, sent_emb=True).pooler_output.cpu()
        # print(base_embeddings.shape)
        
        
        # pos_idx = [idx if label == 1 for idx,label in enumerate(train_labels)]
        ...
        texts = self.eval_dataset["full_text"]
        labels = self.eval_dataset["label"]
        inputs = self.tokenizer(texts, padding=True,max_length=512,
                        truncation=True, return_tensors="pt").to(self.args.device)
        ...
        # self.model.eval()
        with torch.no_grad():
            embeddings = self.model(**inputs, output_hidden_states=True,
                            return_dict=True, sent_emb=True).pooler_output.cpu()
        ...
        #                 pos_res.append((i, j, cosine_sim))
        #             else:
        #                 neg_res.append((i, j, cosine_sim))
        #         pos_top_n = heapq.nlargest(t+1, pos_res, key=lambda x: x[2])
        #         neg_top_n = heapq.nlargest(t+1, neg_res, key=lambda x: x[2])
        ...
        #                 wrong = wrong + 1
        #         # neg = neg + 1
        #     if right/(right+wrong) >= acc:
        #         acc = right/(right+wrong)
        #         best_topn = t+1
        
        self.eval_acc_list.append(acc)
        self.eval_step.append(self.state.global_step)
        
        import matplotlib.pyplot as plt
        # 画出训练过程中eval_acc的变化曲线
        # print(self.eval_step)
        plt.plot(self.eval_step,self.eval_acc_list, label='Train')
        ...
        plt.ylim([0, 1])
        # plt.legend()
        # plt.savefig('../berta-base-p=0.3.png')
        
        if acc > self.best_acc:
            self.best_acc = acc
        metrics = {"eval_acc": acc, "eval_topn": best_topn, "best_acc":self.best_acc, "prob":self.model.prob} 
        self.log(metrics)
